...
// Copyright (C) 2002, 2005, 2006, 2007  Braden McDaniel
//
...
namespace openvrml {
    ...
    class OPENVRML_API scope : boost::noncopyable {
        ...
        std::list<std::shared_ptr<node_type> > node_type_list;
        std::map<std::string, node *> named_node_map;
        const std::string id_;
        const std::shared_ptr<scope> parent_;
    ...
    public:
        ...
        const std::shared_ptr<scope> & parent() const OPENVRML_NOTHROW;

        const std::pair<std::shared_ptr<node_type>, bool>
            add_type(const std::shared_ptr<node_type> & type)
            OPENVRML_THROW1(std::bad_alloc);

        const std::shared_ptr<node_type> &
        find_type(const std::string & id) const;

        const std::shared_ptr<node_type> & first_type() const;
