...
namespace openvrml {
    ...
    protected:
        ...
        public:
            ...
            std::shared_ptr<counted_impl_base> clone() const
                OPENVRML_THROW1(std::bad_alloc);

        private:
            virtual std::shared_ptr<counted_impl_base> do_clone() const
    ...
    private:
        std::shared_ptr<counted_impl_base> counted_impl_;
    ...
    public:
        ...
        static std::shared_ptr<field_value> create(type_id type)
            OPENVRML_THROW1(std::bad_alloc);

        virtual ~field_value() OPENVRML_NOTHROW = 0;

        std::shared_ptr<field_value> clone() const
            OPENVRML_THROW1(std::bad_alloc);
    ...
    private:
        virtual std::shared_ptr<field_value> do_clone() const
            OPENVRML_THROW1(std::bad_alloc) = 0;
    ...
    template <typename ValueType>
    std::shared_ptr<field_value::counted_impl_base>
    field_value::counted_impl<ValueType>::do_clone() const
        OPENVRML_THROW1(std::bad_alloc)
    {
        return std::shared_ptr<counted_impl_base>(
    ...
    {
        if (this != &fv) {
            counted_impl_.reset(fv.counted_impl_->clone().get());
        }
