                    ...
                    std::bad_alloc)
{
    ...
    for (directory_iterator entry(component_path);
        ...
        if (!is_directory(entry->path())) try {
            shared_ptr<component> c(new component(entry->path().string()));
            std::string key = c->id();
            succeeded = this->insert(key, c.get()).second;
            assert(succeeded);
    ...
    OPENVRML_THROW1(std::bad_alloc)
{
    ...
    std::shared_ptr<scope> root_scope(new scope(uri));
    ...
    OPENVRML_THROW1(std::bad_alloc)
{
    ...
    std::shared_ptr<node_type_decls> node_type_descs(new node_type_decls);
    for (map_t::const_iterator entry = this->components_.begin();
         entry != this->components_.end();
         ++entry) try {
        const component & c = local::component_registry_.at(entry->first);
