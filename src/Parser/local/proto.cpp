        ...
        class OPENVRML_LOCAL proto_node : public abstract_proto_node {
            ...
            struct proto_exposedfield_creator {
                proto_exposedfield_creator(
                    ...
                    proto_node & node,
                    std::shared_ptr<openvrml::event_listener> & exposedfield):
            ...
            private:
                ...
                proto_node * node_;
                std::shared_ptr<openvrml::event_listener> * exposedfield_;
            };

            static std::shared_ptr<openvrml::event_listener>
                create_exposedfield(const openvrml::field_value & initial_value,
                                    proto_node & node)
                OPENVRML_THROW1(std::bad_alloc);

            std::shared_ptr<openvrml::scope> proto_scope_;
...
namespace {
    ...
    protected:
        const std::shared_ptr<openvrml::scope> & target_scope;
            ...
            if (already_traversed) {
                ...
                for (node_interface_set::const_iterator interface_ =
                    ...
                    if (type == node_interface::exposedfield_id
                        ...
                        using openvrml::field_value;
                        shared_ptr<field_value> src = n->field(id);
                        shared_ptr<field_value> dest =
        ...
        if (already_traversed) {
            ...
            for (node_interface_set::const_iterator interface_ =
                ...
                if (type == node_interface::exposedfield_id
                    ...
                    using std::find_if;
                    using openvrml::field_value;
                    shared_ptr<const field_value> src_val;
 ...
 *
 * @return a @c std::shared_ptr to a @c proto_eventin<FieldValue>
 *         instance.
 *
 * @exception std::bad_alloc    if memory allocation fails.
 */
std::shared_ptr<openvrml::event_listener>
    ...
    OPENVRML_THROW1(std::bad_alloc)
{
    ...
    std::shared_ptr<openvrml::event_listener> result;
 ...
 *
 * @return a @c std::shared_ptr to a @c proto_eventout<FieldValue> instance.
 *
 * @exception std::bad_alloc    if memory allocation fails.
 */
    ...
    OPENVRML_THROW1(std::bad_alloc)
{
    ...
    std::shared_ptr<openvrml::event_emitter> result;
 ...
 * @param[in] node  @c proto_node.
 *
 * @return a @c std::shared_ptr to a @c proto_exposedfield<FieldValue>
 *         instance.
 *
 * @exception std::bad_alloc    if memory allocation fails.
 */
