...
openvrml::script::direct_output(event_listener & listener,
                                const std::shared_ptr<field_value> & value)
...
/**
 * @fn std::shared_ptr<openvrml::script> openvrml::script_factory::create_script(script_node & node, const std::shared_ptr<resource_istream> & source)
 *
...
class openvrml::script_factory_registry::impl :
    ...
    typedef std::map<std::string, std::shared_ptr<script_factory> >
        factory_map;
...
public:
    ...
    bool register_factory(
        ...
        const std::shared_ptr<script_factory> & factory)
        OPENVRML_THROW2(std::bad_alloc, std::invalid_argument);

    const std::shared_ptr<script_factory>
...
namespace {
    typedef std::map<std::string, std::shared_ptr<openvrml::script_factory> >
        factory_map_t;

    struct OPENVRML_LOCAL add_factory_entry : std::unary_function<std::string,
                                                                  void> {
        add_factory_entry(
            const std::shared_ptr<openvrml::script_factory> & factory,
    ...
    private:
        const std::shared_ptr<openvrml::script_factory> factory_;
...
const std::shared_ptr<openvrml::script_factory>
openvrml::script_factory_registry::impl::
find_using_media_type(const std::string & media_type) const
{
    const factory_map::const_iterator pos =
 ...
 */
const std::shared_ptr<openvrml::node_type>
...
openvrml::script_node::eventout::eventout(const field_value::type_id type,
    ...
    node_(node),
    value_(field_value::create(type).get()),
    modified_(false),
    emitter_(create_emitter(node, *this->value_).get())
{}
...
openvrml::script_node::script_node_type::
do_create_node(const std::shared_ptr<openvrml::scope> & scope,
...
struct OPENVRML_LOCAL openvrml::script_node::script_event_listener_creator {
    script_event_listener_creator(
        ...
        const std::string & id,
        script_node & node,
        std::shared_ptr<openvrml::event_listener> & listener):
        type_(type),
        id_(&id),
        node_(&node),
        listener_(&listener)
...
private:
    ...
    script_node * node_;
    std::shared_ptr<openvrml::event_listener> * listener_;
};
 ...
 * @exception std::bad_alloc    if memory allocation fails.
 */
const std::shared_ptr<openvrml::event_listener>
...
struct OPENVRML_LOCAL openvrml::script_node::script_event_emitter_creator {
    script_event_emitter_creator(
        ...
        std::shared_ptr<openvrml::event_emitter> & emitter):
        node_(&node),
        value_(&value),
        emitter_(&emitter)
    {}
...
private:
    ...
    std::shared_ptr<openvrml::event_emitter> * emitter_;
};
 ...
 * @exception std::bad_alloc    if memory allocation fails.
 */
std::shared_ptr<openvrml::event_emitter>
...
script_node(script_node_metatype & class_,
            const std::shared_ptr<openvrml::scope> & scope,
    ...
    events_received(0)
{
    ...
    for (node_interface_set::const_iterator interface_ = interfaces.begin();
        ...
        shared_ptr<openvrml::field_value> cloned_field_value;
        bool succeeded = false;
    ...
    OPENVRML_THROW1(std::bad_alloc)
{
    ...
    this->script_.reset(this->create_script().get());
 ...
 */
std::shared_ptr<openvrml::script> openvrml::script_node::create_script()
    OPENVRML_THROW2(no_alternative_url, std::bad_alloc)
{
    using std::string;
    using std::vector;

    std::shared_ptr<resource_istream> script_code;
    std::shared_ptr<script_factory> factory;
    ...
    for (vector<string>::const_iterator url(urls.begin());
        ...
        std::shared_ptr<resource_istream> in;
        try {
            //
